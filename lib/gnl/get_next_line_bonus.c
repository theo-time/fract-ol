...
/*   Updated: 2022/12/01 11:30:28 by teliet           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*s_buf[1048564];
	char		*line;

	if (fd < 0 || BUFFER_SIZE <= 0)
		return (NULL);
	s_buf[fd] = get_to_next_line(fd, s_buf[fd]);
	if (!s_buf[fd] || (s_buf[fd] && s_buf[fd][0] == 0))
	{
		free(s_buf[fd]);
		s_buf[fd] = NULL;
		return (NULL);
	}
	line = get_line(&(s_buf[fd]));
	return (line);
}
